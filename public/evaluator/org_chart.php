...
// الشرط: إما ليسوا مسؤولين، أو مسؤولين ولكن لديهم إدارة (يتبعون هيكلاً إدارياً)
$stmt = $pdo->query("
    ...
    AND (
        u.role != 'admin' 
        OR (u.role = 'admin' AND u.department_id IS NOT NULL)
    )
");
$users = $stmt->fetchAll();
...
foreach ($users as $user) {
    ...
    // التحقق من وجود المدير في القائمة المفلترة
    $managerExists = false;
    if ($parentId) {
        foreach ($users as $u) {
    ...
    // إذا كان المدير غير موجود (مثلاً أدمن غير تابع لإدارة وتم استثناؤه)، نجعله في المستوى الأعلى
    if (!$managerExists) {
        $parentId = ''; 
    }
...
<head>
    <meta charset="UTF-8">
    <title>الهيكل التنظيمي - موظف التقييمات</title>
...
<main class="admin-main-content">
    ...
    <div class="alert alert-info">
        <i class="fas fa-info-circle"></i> يعرض هذا الهيكل الموظفين والمسؤولين التابعين لإدارات فقط.
    </div>
...
<script type="text/javascript">
    ...
    function drawChart() {
        ...
        if (chartData.length === 0) {
            document.getElementById('chart_div').innerHTML = '<div class="alert alert-warning">لا توجد بيانات كافية لرسم الهيكل.</div>';
            return;
        }
