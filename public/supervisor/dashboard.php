...
if ($active_cycle) {
    $employees_stmt = $pdo->prepare("
        ...
        WHERE u.supervisor_id = ? AND u.role IN ('employee', 'evaluator')
        ORDER BY u.name
    ");
    $employees_stmt->execute([$active_cycle['id'], $active_cycle['id'], $supervisor_id]);
...
<main class="admin-main-content">
    ...
    <?php else: ?>
        ...
        <div class="card">
            <div class="card-header">
                <i class="fas fa-users"></i> الموظفون التابعون لك (دورة <?= $active_cycle['year'] ?>)
            </div>
            <div class="card-body">
                    ...
                    <table class="table table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th>الإدارة</th>
                                <th>تقييمك</th>
                                <th>تقييم مدير الإدارة</th>
                                <th>الإجراءات</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php if (empty($employees)): ?>
                                <tr><td colspan="7" class="text-center">لا توجد موظفين تابعين لك.</td></tr>
                            <?php else: ?>
    ...
    type: 'pie',
     {
        ...
        datasets: [{
             [<?= $approved_count ?>, <?= $rejected_count ?>, <?= $pending_count ?>, <?= $not_evaluated_count ?>],
            backgroundColor: [
